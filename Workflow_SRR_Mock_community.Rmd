...
![**Schematic a:** Short overlap between the paired-end reads over the target ITS region, **Schematic b:** Complete overlap and extension into the second primer for the paired-end reads over the target ITS region, ](ITS_region_schema.png) 


Schematic a here depicts a scenario where the forward and reverse reads overlap and allows for capturing the entire ITS region and schematic b here depicts a scenario where the forward and reverse read into the second primer and capture the reverse complement at the end of the read.
...
prmr_form <- (c(prmr,cmplmnt,rvrse,rvrse_cmplmnt))
prmr_form <- setNames(prmr_form, c("Forward", "Complement","Reverse","Reverse Complement"))
return(prmr_form)
}
...
The [dataset](https://www.ebi.ac.uk/ena/data/view/PRJNA377530) used here is the Amplicon sequencing library #1, an ITS Mock community constructed by selecting 19 known fungal cultures from the microbial culture collection at the USDA Agricultural Research Service (CFMR) culture collection and sequenced on an Illumina MiSeq using a version 3 (600 cycle) reagent kit. The primers used for this construction are as listed below, we create the complement, reverse and reverse complement forms of the primer BITS and B58S3 using the forward orientation of the primer as the input to the prmr_funcn. 


```{r ITS primers, warning=FALSE, message=FALSE, tidy=TRUE, comment= ""}
BITS <- "ACCTGCGGARGGATCA" ## Define your primer sequence here
B58S3 <- "GAGATCCRTTGYTRAAAGTT"
BITS.form <- prmr_funcn(BITS)
B58S3.form <- prmr_funcn(B58S3)
primer.form <- c(BITS.form, B58S3.form)
B58S3.form ##The output is ordered in the forward, complement, reverse and reverse complement orientations of the ITS4 primer.
```
...
path <- "~/Desktop/Fungal_mock_community" ## CHANGE ME to the directory containing the fastq files.
list.files(path)
```

If the `list.files` listed all the files you expect, identify the FASTQ files pattern for **your** sequence files. Here, the sequence files have the "_1.fastq.gz" and "_2.fastq.gz" format, we use these file patterns to read in the sequence files and perform some string manipulation to get matched lists of the forward and reverse fastq files. 
...
Mock.fnFs <- sort(list.files(path, pattern = "_1.fastq.gz", full.names = TRUE))
Mock.fnRs <- sort(list.files(path, pattern = "_2.fastq.gz", full.names = TRUE))
...
```{r Sequence read in, warning=FALSE, message=FALSE, tidy=TRUE,comment= ""}
...
```{r Primer check, warning=FALSE, message=FALSE, tidy=TRUE, results='hold',comment= ""}
...
Mock.fnFs.cutadapt <- file.path(path, "cutadapt_trimmed", paste0(mock.names,"_R1_trimmed.fastq.gz"))
Mock.fnRs.cutadapt <- file.path(path, "cutadapt_trimmed", paste0(mock.names,"_R2_trimmed.fastq.gz"))
trim.cmd <- "-n 2 -m 50"
primers<-c("-g ACCTGCGGARGGATCA", "-a AACTTTYARCAAYGGATCTC", "-G GAGATCCRTTGYTRAAAGTT", "-A TGATCCYTCCGCAGGT")
...
path.trimmed <- paste0(path, "/", "cutadapt_trimmed")  ##Provide path to the primer trimmed directory
fnFs <- sort(list.files(path.trimmed, pattern = "R1_trimmed.fastq", full.names = TRUE))
fnRs <- sort(list.files(path.trimmed, pattern = "R2_trimmed.fastq", full.names = TRUE))
...
filtFs <- file.path(path.trimmed, "filtered", paste0(sample.names, "_F_filt.fastq.gz"))
filtRs <- file.path(path.trimmed, "filtered", paste0(sample.names, "_R_filt.fastq.gz"))
```
...
```{r Dereplication, warning=FALSE, message=FALSE, tidy=TRUE,comment= ""}
...
```{r seqtab dimension, warning=FALSE,message=FALSE, tidy=TRUE,comment= ""}
dim(seqtab)
```
...
The frequency of chimeric sequences varies substantially from dataset to dataset, and depends on on factors including experimental procedures and sample complexity. Here chimeras make up about 34% of the merged sequence variants, but when we account for the abundances of those variants we see they account for less than 3% of the merged sequence reads.
